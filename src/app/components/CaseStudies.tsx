...
export default function CaseStudies() {
  const [activeIndex, setActiveIndex] = useState(0); // Card 1 active by default on load
  ...
  return (
    <section id="case-studies" className="relative py-24 md:py-32 px-4 max-w-[1180px] mx-auto z-10 select-none">
      {/* 1. Header Section */}
      <div id="case-study-header" className="flex flex-col items-center text-center mb-16">
        <motion.h2
      ...
      {/* 2. Interactive Horizontal Expandable Accordion Slider (3 Cards - CLICK Trigger ONLY) */}
      <div className="flex flex-col lg:flex-row gap-4 h-[650px] lg:h-[480px] w-full items-stretch relative z-10">
          ...
          return (
            <motion.div
              ...
              className={`relative rounded-[24px] overflow-hidden cursor-pointer border border-white/10 flex transition-all duration-500 ${
                isActive
                  ? "lg:flex-[3.5] bg-[#191919] p-6 lg:p-8"
                  : "lg:flex-[0.55] bg-[#191919]/60 hover:bg-[#191919] p-3"
              }`}
            >
              {/* Royal Blue Glow Shadow for Active Card */}
              {isActive && (
                <div
                  className="absolute -bottom-10 -right-10 w-96 h-96 rounded-full pointer-events-none opacity-60 transition-opacity duration-500"
                  style={{
              ...
              {/* CARD LAYOUT */}
              <div className="w-full h-full flex flex-col lg:flex-row gap-6 relative z-10 items-stretch">
                {/* Image Section */}
                <div
                  className={`relative rounded-[20px] overflow-hidden transition-all duration-500 shrink-0 ${
                    isActive
                      ? "w-full lg:w-[48%] h-[240px] lg:h-full"
                      : "w-full h-full"
                  }`}
                >
                  <img
                ...
                <AnimatePresence mode="wait">
                  {isActive && (
                    <motion.div
                      ...
                      className="flex-1 flex flex-col justify-between py-1 px-2"
                    >
                      {/* Result Badges */}
                      <div className="flex flex-col gap-3 mb-6 items-start">
                        {study.badges.map((badge, bIdx) => {
                          ...
                          return (
                            <div
                              key={bIdx}
                              className="inline-flex items-center gap-3 px-4 py-2.5 rounded-full border border-[#4555fd] bg-[#191919] shadow-lg"
                            >
                      ...
                      {/* VIEW CASE STUDY Reusable Button */}
                      <div>
